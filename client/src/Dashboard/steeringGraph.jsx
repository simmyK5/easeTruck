...
const SteeringGraph = ({ userId, userRole }) => {
  ...
  const [steeringAngleRange, setSteeringAngleRange] = useState("0-100");
  ...
  const fetchsteeringAngle = useCallback(async () => {
    try {
      ...
      if (data && typeof data === "object") {
        ...
        setSteeringData(combinedData);
      } else {
        setSteeringData([]);
      }
    } catch (error) {
      console.error('Error fetching steeringAngle data:', error);
      setSteeringData([]);
    } finally {
      setLoading(false);
    }
  ...
  }, [userId, userRole, groupBy, customStartDate, customEndDate, fetchsteeringAngle]);

  const filteredData = steeringData.filter(item => {
    ...
    const [minAccel, maxAccel] = steeringAngleRange.split("-").map(Number);
    if (item.steeringAngle < minAccel || item.steeringAngle > maxAccel) return false;
  ...
  steeringData.forEach(item => {
    item.fullName = `${item.firstName} ${item.lastName}`;
  });


  const uniqueGroups = ["all", ...new Set(steeringData.map(item => item.fullName))];


  console.log("accc", steeringData)
  /*const uniqueGroups = ["all", ...new Set(steeringData.map(item =>item[groupBy]))];
  const uniqueGroups = ["all", ...Object.keys(steeringData)];*/
      ...
      case "bar":
        return (
          <BarChart {...commonProps}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="steeringAngle" fill="#8884d8" />
      ...
      case "area":
        return (
          <AreaChart {...commonProps}>
            ...
            <Tooltip />
            <Legend />
            <Area type="monotone" dataKey="steeringAngle" stroke="#8884d8" fill="#8884d8" />
      ...
      case "scatter":
        return (
          <ScatterChart {...commonProps}>
            ...
            <YAxis dataKey="steeringAngle" name="Steering Angle" />
            <Tooltip cursor={{ strokeDasharray: '3 3' }} />
            <Scatter name="Speed vs Steering Angle" data={filteredData} fill="#8884d8" />
          </ScatterChart>
        );
      case "geolocation":
        return (
      ...
      case "accTable":
        return (
          <Grid item xs={12} style={{ height: 500 }}>
            <Typography variant="h6" gutterBottom>All steering Angle Records</Typography>
            <DataGrid
              rows={rows}
              columns={columns}
              pageSize={10}
  ...
  const downloadExcel = (data) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "steeringData");
  ...
  const columns = [
    ...
    { field: 'steeringAngle', headerName: 'steeringAngle (m/s²)', width: 180 },
    { field: 'speedLimit', headerName: 'Speed Limit', width: 130 },
    { field: 'serialNumber', headerName: 'Truck', width: 130 },
  ];
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Typography variant="h5">steeringAngle Dashboard</Typography>
      </Grid>
      ...
      <Grid item xs={6} md={3}>
        <TextField
          ...
          label="Accel Range (e.g. 0-100)"
          value={steeringAngleRange}
          onChange={e => setSteeringAngleRange(e.target.value)}
        />
      </Grid>
