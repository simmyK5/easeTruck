...
const SecurityAlertGraph = ({ userId, userRole }) => {
  ...
  const fetchAcceleration = useCallback(async () => {
    try {
      console.log("dlala", userId)
      const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/backend/dashboard/securityAlert`, {
        params: {
          userId,
          userRole,
          groupBy,
  ...
  }, [userId, userRole, groupBy, customStartDate, customEndDate, fetchAcceleration]);
  ...
  const uniqueGroups = ["all", ...new Set(sensorData.map(item => item.fullName))];


  console.log("accc", sensorData)
  /*const uniqueGroups = ["all", ...new Set(sensorData.map(item =>item[groupBy]))];
  const uniqueGroups = ["all", ...Object.keys(sensorData)];*/
      ...
      case "area":
        return (
          <AreaChart {...commonProps}>
            ...
            <Tooltip />
            <Legend />
            <Area type="monotone" dataKey="acceleration" stroke="#8884d8" fill="#8884d8" />
      ...
      case "accTable":
        return (
          <Grid item xs={12} style={{ height: 500 }}>
            <Typography variant="h6" gutterBottom>All security alert Records</Typography>
            <DataGrid
              rows={rows}
              columns={columns}
              pageSize={10}
  ...
  const downloadExcel = (data) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "AccelerationData");
  ...
  const downloadPDF = () => {
    ...
    const data = filteredData.map(item => [
      ...
      item.acceleration ? `${item.acceleration} m/s²` : "N/A",
  ...
  const columns = [
    ...
    { field: 'acceleration', headerName: 'Acceleration (m/s²)', width: 180 },
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Typography variant="h5">Secuirty Alert Dashboard</Typography>
      </Grid>
...
export default SecurityAlertGraph;
