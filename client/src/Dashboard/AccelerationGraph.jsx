...
const AccelerationGraph = ({ userId, userRole }) => {
  ...
  const fetchAcceleration = useCallback(async () => {
    try {
      ...
      if (data && typeof data === "object") {
        ...
        setAccelerationData(combinedData);
      } else {
        setAccelerationData([]);
      }
    } catch (error) {
  ...
  const filteredData = accelerationData.filter(item => {
    ...
    if (item.acceleration < minAccel || item.acceleration > maxAccel) return false;
  ...
  const uniqueGroups = ["all", ...new Set(accelerationData.map(item => item.fullName))];


  console.log("accc", accelerationData)
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Typography variant="h5">Acceleration Dashboard</Typography>
      </Grid>
      ...
      <Grid item xs={6} md={3}>
        <TextField
          ...
          value={accelRange}
          onChange={e => setAccelRange(e.target.value)}
        />
      </Grid>
